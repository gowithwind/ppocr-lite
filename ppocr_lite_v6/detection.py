...
from ppocr_lite_v6.utils import log_perf
...
class DetPreProcess:
    ...
    Normalisation:
      v5: (x/255 − 0.5)/0.5  ≡  x × (1/127.5) − 1
      v6: (x/255 − μ) / σ    where μ=[0.485,0.456,0.406], σ=[0.229,0.224,0.225]

    The result is made C-contiguous here so FastONNXRunner never needs
    an extra copy.
    """

    V6_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
    V6_STD  = np.array([0.229, 0.224, 0.225], dtype=np.float32)

    def __init__(self, limit_side_len: int = 960, limit_type: str = "max",
                 model_version: str = "v5") -> None:
    ...
    def __call__(self, img: np.ndarray) -> np.ndarray:
        ...
        if self.model_version == "v6":
            arr = (arr / 255.0 - self.V6_MEAN) / self.V6_STD
        else:
            # v5 normalise: (x/255 − 0.5)/0.5  ==  x/127.5 − 1
