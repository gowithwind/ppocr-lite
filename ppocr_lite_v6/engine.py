...
class PPOCRLite:
    ...
    def __init__(
        ...
        *,
        ...
        rec_batch_size: int = 24,
        use_cls: Optional[bool] = None,
        providers: Optional[List[str]] = None,
    ) -> None:
        cfg = (config or ModelConfig(model_version=model_version)).resolve()
        ...
        # --- Recognition ---
        # v6 rec model doesn't cap width (vs v5's 320)
        rec_max_width = 9999 if model_version == "v6" else 320
        self._rec_pre     = RecPreProcess(max_width=rec_max_width)
        self._rec_batch   = rec_batch_size
        self._rec_session = FastONNXRunner(_load_session(cfg.rec_model, self._providers))
        try:
            self._decoder = _build_decoder(self._rec_session.session, model_version)
        except RuntimeError:
            self._decoder = CTCDecoder.from_file(cfg.dict_path, model_version=model_version)
    ...
    def _detect(self, img: np.ndarray) -> list[BBox]:
        ...
        if self._model_version == "v6":
            # v6: max side = 640
            pre = DetPreProcess(limit_side_len=640, limit_type="max",
                                model_version="v6")
        else:
            if min(h, w) < 960:
        ...
        # v6: boxes are ~2.4x too short vs JS contours — expand vertically only
        if self._model_version == "v6":
            for b in boxes:
                pv = int(b.height * 0.7)
                ph = int(b.height * 0.1)
                b.x = max(0, b.x - ph)
                b.y = max(0, b.y - pv)
                b.width = min(w - b.x, b.width + 2 * ph)
                b.height = min(h - b.y, b.height + 2 * pv)
...
def _build_decoder(session, model_version: str = "v5") -> CTCDecoder:
    meta = session.get_modelmeta().custom_metadata_map
    if "character" in meta:
        return CTCDecoder.from_model_metadata(meta, model_version=model_version)
    raise RuntimeError(
